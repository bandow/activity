...
(function($) {
	...
	$.fn.slotMachine = function(settings){
		...
		function  _appolit(index){
			var appIndex = index<$titles.length || index==undefined ? index : 0;
			var appObj = {
				index: appIndex,
		...
		function _shuffle( count,index ){
			...
			if( count===undefined ){
				...
				if( count>=1 ){
					...
					// _stop(true);
					if(index!=undefined)
						_stop(true,function(){return _appolit(index)});
					else
						_stop(true);
		 ...
		 * @param int count - Number of shuffles (undefined to make infinite animation
		 */
		$slot.shuffle = function( count, oncomplete,index ){

			_forceStop = false;

			_oncompleteShuffling = oncomplete;

			_shuffle(count,index);
